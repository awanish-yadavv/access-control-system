...
const AccessLogsPage = () => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const base = filter !== 'all' ? `/access-logs?result=${filter}&limit=100` : '/access-logs?limit=100';
      const res = await apiGet<{ logs: AccessLog[]; total: number }>(base);
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-border">
              {['Result', 'Card UID', 'Reason', 'Tenant', 'Trace ID', 'Time'].map((h) => (
                <th key={h} className="font-mono text-[9px] tracking-[0.14em] uppercase text-muted-foreground px-4 py-2.5 text-left font-normal">
                  {h}
                </th>
              ))}
            </tr>
            ...
            {loading ? (
              ...
              return (
                <tr key={log.id} className="border-b border-border last:border-0 hover:bg-secondary/50 transition-colors">
                  ...
                  <td className="px-4 py-2.5 font-mono text-[10px] text-muted-foreground">{log.tenantId?.slice(0, 8) ?? '—'}</td>
                  <td className="px-4 py-2.5 font-mono text-[9px] text-muted-foreground tracking-[0.04em]">{log.traceId?.slice(0, 12)}…</td>
